...
from config import (
    ARCHIVO, HOY, HOY_TS,
    C_CIAN, C_CRITICO, C_ALERTA, C_OK, C_GRIS, C_MORADO, C_VERDE2, C_INDIGO,
    PRIO_ORD, PRIO_CLR, PRIO_ICO, EST_CLR, EST_ICO, TIPO_CLR, CAT_CLR,
...
# Wrappers locales que capturan df_raw/df_ter del scope global
def _activas():          return _activas_fn(df_raw)
def estado_general():    return _eg_fn(df_raw)
def tasa_cumplimiento_semana(): return _tcs_fn(df_raw)
def guardar_github(df, df_ter_nuevo=None):
    return _guardar_base(df, df_ter_nuevo=df_ter_nuevo, df_ter_base=df_ter)
